...
import common_flags as common_flags
...
def create_optimizer(hparams):
    ...
    optimizer = tf.compat.v1.train.MomentumOptimizer(hparams.learning_rate, momentum=hparams.momentum)
    ...
    if hparams.optimizer == 'adam':
        ...
        optimizer = tf.compat.v1.train.RMSPropOptimizer(hparams.learning_rate, momentum=hparams.momentum)

    return optimizer


def train(loss, init_fn, hparams):
    """Wraps tf_slim.learning.train to run a training loop.
  Args:
    loss: a loss tensor
  ...
  """
    ...
    if FLAGS.sync_replicas:
        ...
        optimizer = tf.compat.v1.train.SyncReplicasOptimizer(opt=optimizer,
    ...
    train_op = tf_slim.learning.create_train_op(loss, optimizer, summarize_gradients=True, clip_gradient_norm=FLAGS.clip_gradient_norm)
    tf_slim.learning.train(train_op=train_op, logdir=FLAGS.train_log_dir, graph=loss.graph, master=FLAGS.master,
                           is_chief=(FLAGS.task == 0),
                           number_of_steps=FLAGS.max_number_of_steps, save_summaries_secs=FLAGS.save_summaries_secs,
                           save_interval_secs=FLAGS.save_interval_secs, startup_delay_steps=startup_delay_steps,
                           sync_optimizer=sync_optimizer, init_fn=init_fn)
...
def prepare_training_dir():
    if not tf.io.gfile.exists(FLAGS.train_log_dir):
        ...
        if FLAGS.reset_train_dir:
            ...
            tf.compat.v1.gfile.DeleteRecursively(FLAGS.train_log_dir)
            tf.io.gfile.makedirs(FLAGS.train_log_dir)
        else:
...
def main(_):
    ...
    device_setter = tf.compat.v1.train.replica_device_setter(FLAGS.ps_tasks, merge_devices=True)
    with tf.device(device_setter):
        data = data_provider.get_data(dataset, FLAGS.batch_size, augment=hparams.use_augment_input,
        ...
        if FLAGS.show_graph_stats:
            logging.info('Total number of weights in the graph: %s', calculateParameter())
