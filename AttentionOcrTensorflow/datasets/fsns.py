...
class _NumOfViewsHandler(tf_slim.tfexample_decoder.ItemHandler):
    ...
    def tensors_to_item(self, keys_to_tensors):
        return tf.cast(self._num_of_views * keys_to_tensors[self._original_width_key] / keys_to_tensors[self._width_key], dtype=tf.int64)
...
def get_split(split_name, dataset_dir=None, config=None):
  ...
  """
    ...
    keys_to_features = {
        ...
        'image/width':
            tf.compat.v1.FixedLenFeature([1], tf.int64, default_value=zero),
        'image/orig_width':
            tf.compat.v1.FixedLenFeature([1], tf.int64, default_value=zero),
        'image/class':
            tf.compat.v1.FixedLenFeature([config['max_sequence_length']], tf.int64),
        'image/unpadded_class':
            tf.compat.v1.VarLenFeature(tf.int64),
        'image/text':
            tf.compat.v1.FixedLenFeature([1], tf.string, default_value=''),
    }
    items_to_handlers = {
        'image':
            tf_slim.tfexample_decoder.Image(
                shape=config['image_shape'],
                image_key='image/encoded',
                format_key='image/format'),
        'label':
            tf_slim.tfexample_decoder.Tensor(tensor_key='image/class'),
        'text':
            tf_slim.tfexample_decoder.Tensor(tensor_key='image/text'),
        'num_of_views':
            _NumOfViewsHandler(
    ...
    decoder = tf_slim.tfexample_decoder.TFExampleDecoder(keys_to_features, items_to_handlers)
    ...
    return tf_slim.dataset.Dataset(
